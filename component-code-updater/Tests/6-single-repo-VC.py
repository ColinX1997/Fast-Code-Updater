...
def download_files(updates):
    ...
    for local_file, remote_file in updates:
        ...
        new_project_version = extract_version([remote_file], "name_version")
        old_project_version = local_file.split()[1]
        extract_to_version_folder(
            download_file_path, name, new_project_version[name], old_project_version
        )


def extract_to_version_folder(rpm_path, name, new_version, old_version):
    # 使用7-zip解压文件
    ...
    os.remove(rpm_path)
    extract_cpio_files(extract_rpm_to_path, new_version, old_version)


def extract_cpio_files(extract_rpm_to_path, new_version, old_version):
    # 获取下载文件夹中的所有文件
    ...
    for file_name in files:
        if file_name.endswith(".cpio"):
            ...
            # 创建Version文件夹
            extract_cpio_to_path = os.path.join(extract_rpm_to_path, new_version)
            ...
            overwrite_files(
                extract_rpm_to_path,
                extract_cpio_to_path,
                new_version,
            )


def overwrite_files(root_path, src_path, new_version):
    # 源文件夹和目标文件夹的路径

    # 遍历源文件夹中的所有文件
    for filename in os.listdir(src_path):
        # 构建完整的文件路径
        src_folder = os.path.join(src_path, filename)
        dst_folder = os.path.join(root_path, filename)
        if os.path.exists(dst_folder):
            shutil.rmtree(dst_folder)

        # 复制文件夹
        shutil.copytree(src_folder, dst_folder, ignore=ignore_pyc_files)
    print("数据迁移完成")
    try:
        shutil.rmtree(src_path)
    except PermissionError:
        print(f"请手动删除： {src_folder}")
    git_add(root_path, new_version)


# 定义一个忽略函数，用于过滤 .pyc 文件
